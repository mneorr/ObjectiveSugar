...
- (NSRange)rangeFromString:(NSString *)string {
  ...
  if ([string containsString:@"..."]) {
    range.length = isBackwardsRange(string) ? (self.count - 2) - range.length
                                            : range.length - range.location;

  } else if ([string containsString:@".."]) {
    range.length = isBackwardsRange(string) ? (self.count - 1) - range.length
...
- (NSArray *)sortedArrayByKey:(NSString *)key ascending:(BOOL)ascending {
  return [self sortedArrayByKeys:@[ key ] ascending:@[ @(ascending) ]];
}

- (NSArray *)sortedArrayByKeys:(NSArray *)keys
                     ascending:(NSArray *)ascendingArray {
  if (keys.count != ascendingArray.count) {
    return self;
  }
